...
def _plot_parity(y_true: np.ndarray, y_pred: np.ndarray, out_path: Path, title: str) -> None:
...
def _plot_residual_hist(y_true: np.ndarray, y_pred: np.ndarray, out_path: Path, title: str) -> None:
    import matplotlib.pyplot as plt

    residual = y_pred - y_true
    plt.figure(figsize=(5, 4))
    plt.hist(residual, bins=40, alpha=0.7, color="tab:blue")
    plt.title(title)
    plt.xlabel("Residual")
    plt.ylabel("Count")
    plt.tight_layout()
...
def _plot_pull(y_true: np.ndarray, y_pred: np.ndarray, out_path: Path, title: str) -> None:
    import matplotlib.pyplot as plt
    ...
    pull = residual / sigma
    plt.figure(figsize=(5, 4))
    plt.hist(pull, bins=40, alpha=0.7, color="tab:green")
    plt.title(title)
...
def _plot_pred_compare(
    y_pytorch: np.ndarray, y_hls: np.ndarray, out_path: Path, title: str
) -> None:
    import matplotlib.pyplot as plt
...
def compile_and_compare(hls_model: Any, cfg: Dict[str, Any]) -> Dict[str, Dict[str, float]]:
    ...
    out_dir = io.ensure_dir(cfg["model"]["output_dir"])
    ...
    y_pt = _predict_pytorch(pt_model, X_test)
